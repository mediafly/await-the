...
module.exports = class Limiter extends EventEmitter {
    constructor(collection = [], task = DEFAULT_TASK, options = {}) {
        ...
        this.task = async (currentValue, currentKey) => {
            return task(currentValue, currentKey);
        };
    }
    ...
    /**
     * Get the next item to resolve and resolve it.
     * @private
     * @returns {undefined}
     */
    iterate() {
    ...
    /**
     * When an iteration of running a task is complete emit an error or
     * an iteration event.
     *
    ...
    emitIteration(key, { resultValue, error, index } = {}) {
        ...
        // if we are done emit
        if (this.numberDone >= _.size(this.collection) || (this.stopped && !this.stopEventEmitted)) {
            this.emit('done');
    ...
    /**
     * Register an emission listener on a given channel
