 ...
 * @param {...*} args Variadic arguments to send to the function, _excluding_ the callback.
 * @returns {*}
 */
module.exports = async (fn, ...args) => {
    if (_.isArray(fn)) {
        ...
        if (!_.isFunction(obj[objFnName])) {
            throw new Error(
                `Expected second value of array argument to 'result()' to be the name of a function on the object; instead got: ${
                    obj[objFnName]
                } (function name: ${objFnName})`
            );
        }
